<template>
  <div class="theme-switcher" :class="variant === 'drawer' ? 'theme-switcher--drawer' : 'theme-switcher--toolbar'">
    <button
    ...
    >
      <span class="theme-switcher__toolbar-icon" aria-hidden="true">
        <SvgSunIcon v-if="themeStore.mode === 'light'" width="20px" height="20px" />
        <SvgMoonIcon v-else-if="themeStore.mode === 'dark'" width="20px" height="20px" />
        <SvgComputerIcon v-else width="20px" height="20px" />
      </span>
    </button>
    <button
      v-else
    ...
    <PopupsAppModal :open="modalOpen" :title="t('choose_theme')" @close="modalOpen = false">
      <template #content>
...
const options = computed(() => [
  ...
  { value: 'system' as ThemeMode, label: t('system') },
]);

const currentModeLabelLong = computed(() => {
  const found = options.value.find(o => o.value === themeStore.mode);
  return found ? found.label : t('system');
});

const toolbarAriaLabel = computed(() => `${currentModeLabelLong.value}. ${t('choose_theme')}`);
