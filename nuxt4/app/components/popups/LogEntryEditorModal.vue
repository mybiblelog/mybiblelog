...
import LogEntryEditorForm from '~/components/forms/LogEntryEditorForm.vue';
...
const modalTitle = computed(() =>
  logEntryEditorStore.logEntry?.id ? t('edit_entry') : t('add_entry'),
);

const handleClose = () => {
  logEntryEditorStore.closeEditor({
    confirmMessage: t('messaging.are_you_sure_close_editor'),
  });
};
...
const handleSave = async () => {
  ...
  if (result && entryDate) {
    ...
    if (lookBackDate && entryDate < lookBackDate) {
      dialogStore.alert({ message: t('messaging.entry_before_tracker_start_date') });
    }
  }
};
</script>
