...
- [ ] `margin`, `padding`, and `gap` use the `--mbl-space-*` scale (`3xs`/`2xs`/`xs`/`sm`/`md`/`lg`/`xl`/`2xl`/`3xl`/`4xl`) rather than raw `px`/`rem` literals. Page-level chrome uses the fluid `--mbl-page-*` tokens instead of a fixed step. `border-radius` uses `--mbl-radius-*`. `box-shadow` uses a `--mbl-shadow-*` / `--mbl-ring-*` / `--mbl-focus-ring` token — never hand-written offsets, even when the color is tokenized, since raw geometry does not pick up the dark-theme overrides. All four are enforced by `declaration-property-value-allowed-list` in `web/.stylelintrc.json`, so `npm run -w web lint:css` is the check.
...
- [ ] Typography values use `--mbl-*` tokens rather than hard-coded literals. `z-index` values use the `--z-index-*` tokens from `tokens.css`. (Spacing, radius and shadows are P0 above — stylelint enforces those.)
- [ ] The `box-shadow` token matches the element's *role*, not just a plausible size: `--mbl-shadow-card` / `-card-raised` / `-card-hover` / `-card-pressed` for resting surfaces, `-popover` for dropdowns/menus/tooltips/toasts, `-floating` for the FAB and install prompt, `-modal` for centered modals, `-panel` for edge-anchored sheets and drawers. Reaching past a role alias to a raw `--mbl-shadow-1..4` level, or to `--mbl-shadow-cta` outside marketing content, is a finding. See the block comment above the ladder in `tokens.css`.
