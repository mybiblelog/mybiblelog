 ...
 *   splash-icon.png             1024x1024  transparent bg (splash plugin supplies bg color)
 *   android-icon-foreground.png  512x512   transparent bg, logo in ~66% adaptive safe zone
 *   android-icon-background.png  512x512   solid white
 *   android-icon-monochrome.png  432x432   white alpha silhouette, ~66% safe zone
...
const renderLogo = async (svgText, boxSize) => {
  // Rasterize the full 100x100 viewBox at high density, then fit into boxSize.
...
const main = async () => {
  ...
  await composeOnCanvas(await renderLogo(svgText, 972), 1024, TRANSPARENT, "splash-icon.png");
  await composeOnCanvas(await renderLogo(svgText, 338), 512, TRANSPARENT, "android-icon-foreground.png");
  await composeOnCanvas(await renderLogo(monoSvgText, 285), 432, TRANSPARENT, "android-icon-monochrome.png");
