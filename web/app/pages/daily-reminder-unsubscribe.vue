...
onMounted(async () => {
  ...
  try {
    ...
    const { data } = await $http.post<{ email: string }>(`/api/reminders/daily-reminder/unsubscribe/${code}`);
    complete.value = true;
...
<i18n lang="json">
{
  ...
  "ko": {
    ...
    "client_error": {
      ...
      "p3": "가장 최근 수신된 이메일에 포함된 수신 거부 링크를 클릭하거나 설정을 변경해 주세요."
    },
    "server_error": {
  ...
  "uk": {
    ...
    "server_error": {
      ...
      "p2": "Це стається, коли хтось відписався, а потім знову підписався, створивши нову підписку.",
      "p3": "Будь ласка, перейдіть за посиланням для відписки в останньому електронному листі або увійдіть, щоб оновити свої налаштування."
    },
