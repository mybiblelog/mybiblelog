        ...
        <form>
          <div class="mbl-field">
            <label class="mbl-checkbox">
              <input v-model="understand.logEntries" type="checkbox" :disabled="!mounted"> {{ t('understand.log_entries') }}
            </label>
          </div>
          <div class="mbl-field">
            <label class="mbl-checkbox">
              <input v-model="understand.export" type="checkbox" :disabled="!mounted"> {{ t('understand.export') }}
            </label>
          </div>
          <div class="mbl-field">
            <label class="mbl-checkbox">
              <input v-model="understand.notes" type="checkbox" :disabled="!mounted"> {{ t('understand.notes') }}
            </label>
          </div>
          <div class="mbl-field">
            <label class="mbl-checkbox">
              <input v-model="understand.permanent" type="checkbox" :disabled="!mounted"> {{ t('understand.permanent') }}
            </label>
          </div>
        </form>
        <button
          class="mbl-button mbl-button--primary"
          :disabled="!mounted || !fullyUnderstands || formBusy"
          @click="deleteAccount"
        >
          {{ t('delete_my_account') }}
        </button>
...
const { t } = useI18n();
useHead({ title: () => t('title'), meta: [{ name: 'robots', content: 'noindex' }] });
...
const fullyUnderstands = computed(() =>
  understand.logEntries &&
  understand.export &&
  understand.notes &&
  understand.permanent,
);
...
async function deleteAccount() {
  ...
  formBusy.value = true;
  try {
    await $http.delete('/api/settings/account');
    await authStore.logout();
  }
  catch {
    toastStore.add({ type: 'error', text: t('unable_to_delete') });
