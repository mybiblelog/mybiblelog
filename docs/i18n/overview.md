...
## Supported locales and language tags

[`shared/platform/i18n.ts`](../../shared/platform/i18n.ts) only stores a locale's **code** and display **name** — it does not store a BCP 47 language tag. Nothing in the codebase reads a per-locale tag: hreflang links are generated by [`web/app/composables/useContentSeo.ts`](../../web/app/composables/useContentSeo.ts) from the bare codes in `runtimeConfig.public.locales`, not from a dialect-specific tag, and `@nuxtjs/i18n` v10's own SEO features (which take an optional `language` field on each locale object) aren't configured either.

That means the exact dialect behind each code lives only here, as documentation. Anyone adding UI copy, Bible-translation defaults, email content, or date/number formatting for a locale should use the matching tag below rather than guessing a country:

| Code | Language | Tag | Dialect |
|------|----------|-----|---------|
| `en` | English | `en-US` | American English |
| `de` | Deutsch | `de-DE` | Standard German (Germany) |
| `es` | Español | `es-419` | Latin American Spanish — **not** `es-ES` (Spain) |
| `fr` | Français | `fr-FR` | Standard French (France) |
| `ko` | 한국어 | `ko-KR` | Korean (South Korea) |
| `pt` | Português | `pt-BR` | Brazilian Portuguese — **not** `pt-PT` (Portugal) |
| `uk` | українська | `uk-UA` | Ukrainian (Ukraine) |

If a future feature needs a real BCP 47 tag at runtime (a `language` field for `@nuxtjs/i18n`'s SEO features, for example), source it from this table so `es` and `pt` don't regress to the European dialect.

## Related files

| File | Role |
|------|------|
| [`shared/platform/i18n.ts`](../../shared/platform/i18n.ts) | Supported locale codes and display names (see [Supported locales and language tags](#supported-locales-and-language-tags) above for the BCP 47 tag behind each code — not stored in code) |
